...
int nAnonymizeJetrobeXAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JetrobeX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JetrobeX
    // Mac: ~/Library/Application Support/JetrobeX
    // Unix: ~/.JetrobeX
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JetrobeX";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "JetrobeX.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "JetrobeXd.pid"));
