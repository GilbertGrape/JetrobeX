...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeJetrobeXAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeJetrobeXAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeJetrobeXAmount = strAnonymizeJetrobeXAmount.remove(strAnonymizeJetrobeXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeJetrobeXAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeJetrobeXAmount*COIN) nMaxToAnonymize = nAnonymizeJetrobeXAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeJetrobeXAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeJetrobeXAmount));
        strAnonymizeJetrobeXAmount = strAnonymizeJetrobeXAmount.remove(strAnonymizeJetrobeXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeJetrobeXAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
