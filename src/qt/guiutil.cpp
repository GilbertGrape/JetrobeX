...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("JetrobeX"))
        return false;
...
{
    // Convert JetrobeX:// to JetrobeX:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("JetrobeX://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "JetrobeX:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "JetrobeX.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for JetrobeX.lnk
...
{
    return GetAutostartDir() / "JetrobeX.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a JetrobeX.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=JetrobeX\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the JetrobeX app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add JetrobeX app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("JetrobeX-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  JetrobeX-qt [" + tr("command-line options") + "]                     " + "\n";
