...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeJetrobeXAmount"))
        settings.setValue("nAnonymizeJetrobeXAmount", 1000);
    nAnonymizeJetrobeXAmount = settings.value("nAnonymizeJetrobeXAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeJetrobeXAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeJetrobeXAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeJetrobeXAmount:
            return QVariant(nAnonymizeJetrobeXAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeJetrobeXAmount:
            nAnonymizeJetrobeXAmount = value.toInt();
            settings.setValue("nAnonymizeJetrobeXAmount", nAnonymizeJetrobeXAmount);
            emit AnonymizeJetrobeXAmountChanged(nAnonymizeJetrobeXAmount);
            break;
        default:
            break;
        }
