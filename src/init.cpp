...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: JetrobeX.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: JetrobeXd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeJetrobeXamount=<n> " + _("Keep N JetrobeX anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. JetrobeX is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. JetrobeX is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("JetrobeX version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "JetrobeX server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of JetrobeX") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart JetrobeX to complete") << "\n";
    ...
    nAnonymizeJetrobeXAmount = GetArg("-anonymizeJetrobeXamount", 0);
    if(nAnonymizeJetrobeXAmount > 999999) nAnonymizeJetrobeXAmount = 999999;
    if(nAnonymizeJetrobeXAmount < 2) nAnonymizeJetrobeXAmount = 2;
    ...
    LogPrintf("Anonymize JetrobeX Amount %d\n", nAnonymizeJetrobeXAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1JetrobeX+1000 == (.1JetrobeX+100)*10
       10JetrobeX+10000 == (1JetrobeX+1000)*10
    */
